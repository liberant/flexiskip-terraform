...
class ProductOrder {
  constructor(data, isHeader = false) {
    ...
    this.number = trimValue(number);
    this.email = trimValue(email);
    this.address = trimValue(address);
    this.discountCode = trimValue(discountCode);
    this.productCode = trimValue(productCode) === '' ? null : trimValue(productCode);
    this.quantity = isHeader ? trimValue(quantity) : parseInt(trimValue(quantity), 10);
